...
The Stratis Full Node uses a Proof-of-Stake (PoS) algorithm in its consensus protocol. This means that miners must "stake" STRAT to gain the right to create blocks. The more STRAT that a miner stakes, the greater their chance of being able to write a block. Staking STRAT is a simple process, which anyone who has a wallet and owns some STRAT can do. As you will learn, the Full Node randomizes block creator selection to ensure that the individual staking the most STRAT does not have a permanent advantage.
